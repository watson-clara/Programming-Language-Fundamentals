...
my $STOP_WORDS = 0;           # flag - filter stop words, 0 (off) | 1 (on)
...
my @tracks = ();
# hash of hashes for big ram counts
my %counts = ();
...
sub comments {
	...
	for @tracks -> $title { 
		...
		## Assign to $_ with smart matcher (~~)
		########################## 
		$_ = $title;
...
sub stopwords {
	...
	for @tracks -> $title { 
		...
		$_ ~~ s:g:i/<|w>a<|w>\x[0020]//;

		# Repeat for the other stopwords
		$_ ~~ s:g:i/<|w>an<|w>\x[0020]//;
		$_ ~~ s:g:i/<|w>and<|w>\x[0020]//;
		$_ ~~ s:g:i/<|w>by<|w>\x[0020]//;
		$_ ~~ s:g:i/<|w>for<|w>\x[0020]//;
		$_ ~~ s:g:i/<|w>from<|w>\x[0020]//;
		$_ ~~ s:g:i/<|w>in<|w>\x[0020]//;
		$_ ~~ s:g:i/<|w>of<|w>\x[0020]//;
		$_ ~~ s:g:i/<|w>on<|w>\x[0020]//;
		$_ ~~ s:g:i/<|w>or<|w>\x[0020]//;
		$_ ~~ s:g:i/<|w>out<|w>\x[0020]//;
		$_ ~~ s:g:i/<|w>the<|w>\x[0020]//;
		$_ ~~ s:g:i/<|w>to<|w>\x[0020]//;
		$_ ~~ s:g:i/<|w>with<|w>\x[0020]//;
		##########################
		# Add the edited $title to the new array of titles
		@filteredtitles.push: $_;
	}
...
sub mcw {
	...
	for %counts.keys -> $key
	{
		if %counts{$word}{$key}:exists
		{
			if %counts{$word}{$key} > $i
			{
				if %word_history{$key} != 1
				{
					$i = %counts{$word}{$key};
					$best_word = $key;
				}
